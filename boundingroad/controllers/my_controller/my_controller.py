...
with open(r'..\roadAngles.pickle', 'rb') as handle:
    road_angles = pickle.load(handle)
...
while driver.step() != -1:
    ...
    if graph_coords[current_target] == road_coords[current_target]:
        target_radius = 3
        if target_bearing < front_left_max  and target_bearing > front_left_min:
            angle = min_left_angle
    if road_angles[current_target] != 0:
        if target_bearing > front_left_min  or target_bearing < front_right_max:
    ...
    if stop == 0:
        if graph_coords[current_target] != road_coords[current_target]:
            if ds_values[3] == min_ds:
                #print("yes")
                if target_bearing <= front_right_max or target_bearing >= front_left_min:
                    angle = max_right_angle
        ...
        if target_bearing >= front_right_max and target_bearing < rear:
            ...
            speed = -2 * min_speed
            #print("jobbhat")
        if target_bearing >= rear and target_bearing <= front_left_min:
            angle = max_right_angle
            speed = -2 * min_speed
            #print("balhat")
        if target_bearing > front_right_min and target_bearing < front_right_max:
            speed = ((target_bearing - front_right_min) / (front_right_max - front_right_min)) * (min_speed - max_speed) + max_speed
            angle = ((target_bearing - front_right_min) / (front_right_max - front_right_min)) * (max_right_angle - min_right_angle) + min_right_angle
            #print("jobb")
        if target_bearing < front_left_max  and target_bearing > front_left_min + (front_left_max - front_left_min) / 2:
            speed = ((target_bearing - front_left_min) / (front_left_max - front_left_min)) * (max_speed - min_speed) + min_speed
            angle = (((target_bearing - front_left_min) / (front_left_max - front_left_min)) * (min_left_angle - max_left_angle) + max_left_angle)/2
            #print("balelol")
        if target_bearing < front_left_min + (front_left_max - front_left_min) / 2  and target_bearing > front_left_min:
            ...
            #print("bal")
        if target_bearing >= front_left_max and target_bearing <= front_right_min:
            #turn = 0
            angle = 0
            #print("egyenes")
        ...
        if ds_values[3] > min_ds and ds_values[3] > ds_values[2]:
            ...
            if target_bearing >= front_right_max and target_bearing <= front_left_min:
                ...
                angle = max_left_angle
            #print("jobbelol")
        if ds_values[1] > min_ds and ds_values[1] > ds_values[4]:
            angle = ((ds_values[1] - min_ds) / (max_ds - min_ds)) * (max_right_angle - min_right_angle) + min_right_angle
            if target_bearing >= front_right_max and target_bearing <= front_left_min:
                speed = -2
                angle = max_right_angle
            #print("bal")
        if ds_values[4] > min_ds and ds_values[4] > ds_values[1]:
            angle = ((ds_values[4] - min_ds) / (max_ds - min_ds)) * (max_left_angle + min_left_angle) - min_left_angle
            if target_bearing >= front_right_max and target_bearing <= front_left_min:
                speed = -2
                angle = max_left_angle
            #print("jobb")
        if ds_values[0] > max_ds / 2 and ds_values[0] > ds_values[5]:
            angle = ds_values[0] * max_ds
            speed = ds_values[0] * max_speed
            #print("balhatul")
        if ds_values[5] > max_ds / 2 and ds_values[5] > ds_values[0]:
            angle = ds_values[5] * -max_ds
            speed = ds_values[5] * max_speed
            #print("jobbhatul")
